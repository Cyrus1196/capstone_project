...
if (! $program) {
    throw new RuntimeException('Bachelor of Science in Nursing program was not found.');
}

$choices = ['SCX 010', 'GEN 010', 'ELEC 1'];
...
foreach ($choices as $code) {
    if (! isset($choiceSubjects[$code])) {
        throw new RuntimeException("Missing Nursing GEC elective subject: {$code}");
    }
}

$slotRows = [
    ['slot_name' => 'GEC Elective 1', 'current_subject_code' => 'SCX 010'],
    ['slot_name' => 'GEC Elective 2', 'current_subject_code' => 'GEN 010'],
    ['slot_name' => 'GEC Elective 3', 'current_subject_code' => 'ELEC 1'],
];
...
DB::transaction(function () use ($program, $choiceSubjects, $slotRows, &$updatedCurriculumRows, &$assignedChoices): void {
    foreach ($slotRows as $slotRow) {
        ...
        if (! $curriculum) {
            throw new RuntimeException("Missing BSN curriculum row for {$slotRow['current_subject_code']}");
        }
