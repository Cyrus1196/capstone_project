...
class UserController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $users = TblUser::with('role')->get();
            
            // Manually load dean profiles to avoid relationship issues
            foreach ($users as $user) {
                if ($user->role && $user->role->role_name === 'Dean') {
                    $deanProfile = DeanProfile::where('user_id', $user->user_id)
                        ->with('program')
                        ->first();
                    $user->deanProfile = $deanProfile;
                }
            }
    ...
    public function show(Request $request, $id)
    {
        ...
        $user = TblUser::with(['role', 'deanProfile.program'])->findOrFail($id);
