...
/** Matches backend AuthUnitHelpers::validatePasswordStrength (min 8 + special char). */
function getPasswordStrengthError(password, { required = true } = {}) {
  ...
  if (!/[^A-Za-z0-9]/.test(pwd)) {
    return 'The password must include a special character.';
  }
  return '';
}

/** Default first-login password: {firstname}123 (lowercase, no spaces). */
...
const UserManagement = ({ userScope = 'staff' }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const creatingStudent = isStudentRole() && !editingUser;
      if (creatingStudent) {
        ...
        submitData.email = studentId;
        submitData.password = buildDefaultStudentPassword(firstName);
        setPasswordFieldError('');
      } else {
        if (isStudentRole() && submitData.student_id_number) {
          submitData.email = String(submitData.student_id_number).trim();
        }
        const passwordRequired = !editingUser;
        const passwordErr = getPasswordStrengthError(submitData.password, {
        ...
        <div className="modal-overlay">
          <div className="modal">
            ...
            <form onSubmit={handleSubmit} className="modal-form">
              ...
              {!(isStudentRole() && !editingUser) && (
                <>
                  <div className="form-group">
                    <label>{isStudentRole() ? 'Username (Student ID)' : 'Email'}</label>
                    <input
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setPasswordFieldError(
                          getPasswordStrengthError(nextPassword, { required: !editingUser })
                        );
                      }}
                      onBlur={() => {
                        setPasswordFieldError(
                          getPasswordStrengthError(formData.password, { required: !editingUser })
                        );
                      }}
                    ...
                    />
                    {passwordFieldError ? (
                      <span className="field-error">{passwordFieldError}</span>
                    ) : (
                      <span className="field-hint">At least 8 characters and one special character.</span>
                    )}
                  </div>
                </>
              )}
