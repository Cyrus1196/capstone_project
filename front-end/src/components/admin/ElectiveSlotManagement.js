...
const ElectiveSlotManagement = ({ lockedProgramId = null } = {}) => {
  ...
  const handleEdit = (slot) => {
    ...
    setFormData({
      ...
      prerequisite_slot_id:
        slot.prerequisite_slot_id ?? slot.prerequisiteSlot?.elective_slot_id ?? '',
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...formData,
        prerequisite_slot_id:
          formData.prerequisite_slot_id === '' || formData.prerequisite_slot_id == null
            ? null
            : Number(formData.prerequisite_slot_id),
      };
        ...
        <table className="data-table">
          ...
          <tbody>
            {programScopedSlots.length === 0 ? (
              <tr>
                <td colSpan="8" className="no-data">No elective slots found</td>
              </tr>
            ) : filteredSlots.length === 0 ? (
              <tr>
                <td colSpan="8" className="no-data">No elective slots match your search</td>
              </tr>
            ) : (
              paginatedSlots.map((slot) => (
                <tr key={slot.elective_slot_id}>
                  ...
                  <td>
                    {slot.prerequisiteSlot?.slot_name ||
                      (slot.prerequisite_slot_id
                        ? `Slot #${slot.prerequisite_slot_id}`
                        : '—')}
                  </td>
                  <td>
        ...
        <div className="modal-overlay" onClick={() => {
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="modal-form">
              ...
              <div className="form-group">
                <label htmlFor="esm-slot-prereq">Prerequisite slot</label>
                <select
                  id="esm-slot-prereq"
                  value={
                    formData.prerequisite_slot_id === '' || formData.prerequisite_slot_id == null
                      ? ''
                      : String(formData.prerequisite_slot_id)
                  }
                  onChange={(e) =>
                    setFormData({ ...formData, prerequisite_slot_id: e.target.value })
                  }
                >
                  ...
                  {(slots || [])
                    .filter((s) => {
                      if (
                        editingItem &&
                        String(s.elective_slot_id) === String(editingItem.elective_slot_id)
                      ) {
                        return false;
                      }
                      const programId = formData.program_id || scopedProgramId;
                      if (!programId) return true;
                      return (
                        String(s.program_id ?? s.program?.program_id ?? '') === String(programId)
                      );
                    })
                    .map((s) => (
                      <option key={s.elective_slot_id} value={String(s.elective_slot_id)}>
                        {s.slot_name}
                      </option>
                    ))}
                </select>
                <p style={{ margin: '6px 0 0', color: '#64748b', fontSize: '12px' }}>
                  Example: set IT Electives 2 / 3 / 4 → prerequisite <strong>IT Electives 1</strong>. Guest
                  simulation uses this binding.
                </p>
              </div>
