...
const AcademicManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (activeTab === 'other-school-subjects' && !editingItem) {
      ...
      const normOssCode = (c) => String(c || '').trim().toLowerCase();
      const existingOss = lookupData.otherSchoolSubjects || [];
      const seenBatchCodes = new Set();
      for (const r of toSave) {
        const key = normOssCode(r.code);
        if (seenBatchCodes.has(key)) {
          await swalError(
            'Duplicate subject code',
            `Subject code "${r.code}" appears more than once in this form. Remove the duplicate row.`
          );
          return;
        }
        seenBatchCodes.add(key);
        const clash = existingOss.find(
          (s) => String(s.school_id) === String(schoolId) && normOssCode(s.subject_code) === key
        );
        if (clash) {
          await swalError(
            'Subject already exists',
            `Subject code "${clash.subject_code}" already exists for this school.`
          );
          return;
        }
      }
    ...
    try {
      if (activeTab === 'other-school-subjects' && editingItem) {
        const normOssCode = (c) => String(c || '').trim().toLowerCase();
        const sid = formData.school_id;
        const clash = (lookupData.otherSchoolSubjects || []).find(
          (s) =>
            String(s.school_id) === String(sid) &&
            normOssCode(s.subject_code) === normOssCode(formData.subject_code) &&
            String(s.other_subject_id) !== String(editingItem.other_subject_id)
        );
        if (clash) {
          const msg = `Subject code "${clash.subject_code}" already exists for this school.`;
          setError(msg);
  ...
  const renderTable = () => {
    const dataKey = activeTab.replace(/-/g, '_');
    const allRows = data[dataKey] || [];
    const totalItems = allRows.length;
    const totalPages = Math.max(1, Math.ceil(totalItems / listPageSize) || 1);
    const page = Math.min(Math.max(1, listPage), totalPages);
    const start = (page - 1) * listPageSize;
    const currentData = allRows.slice(start, start + listPageSize);
